...
var _ = BeforeSuite(func() {
	...
	controller = provisioningdynamic.NewController(env.Client, events.NewRecorder(&record.FakeRecorder{}), cloudProvider, cluster, fakeClock)
	prov = provisioning.NewProvisioner(env.Client, events.NewRecorder(&record.FakeRecorder{}), cluster)
...
var _ = Describe("Provisioning", func() {
	...
	It("should provision nodes for pods with supported node selectors", func() {
		...
		ExpectApplied(ctx, env.Client, nodePool)
		ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, controller, schedulable...)
		for _, pod := range schedulable {
			ExpectScheduled(ctx, env.Client, pod)
		}
		ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, controller, unschedulable...)
	...
	It("should schedule all pods on one inflight node when node is in deleting state", func() {
		...
		bindings := ExpectProvisionedNoBinding(ctx, env.Client, cluster, cloudProvider, controller, test.UnschedulablePod(), test.UnschedulablePod())
		nodes := &corev1.NodeList{}
		Expect(env.Client.List(ctx, nodes)).To(Succeed())
	...
	Context("Volume Topology Requirements", func() {
		...
		It("should schedule valid pods when a pod with an invalid pvc is encountered (pvc)", func() {
			...
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, controller, invalidPod)
			pod := test.UnschedulablePod()
			ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, controller, pod)
	...
	Context("Preferential Fallback", func() {
		...
		Context("Preferences", func() {
			...
			DescribeTable("should ignore node preferredDuringSchedulingIgnoredDuringExecution affinity", func(topologyKey string) {
				...
				ExpectApplied(ctx, env.Client, test.NodePool())
				ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, controller, pod1, nodePreferencePod)
		...
		Context("Ignore Preferences", func() {
			...
			It("should ignore node affinity preferences", func() {
				...
				ExpectApplied(ctx, env.Client, test.NodePool())
				ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, controller, zone1Pod, zone2Pods[0], zone2Pods[1], nodeAffinityPod)
			...
			DescribeTable("should ignore pod affinity preferences", func(topologyKey string) {
				...
				ExpectApplied(ctx, env.Client, test.NodePool())
				ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, controller, zone1Pod, zone2Pods[0], zone2Pods[1], affinityPod)
